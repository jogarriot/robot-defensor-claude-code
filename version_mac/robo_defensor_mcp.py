...
Bypasses macOS CoreBluetooth entirely — sends raw HCI commands directly to
the ASUS USB-BT500 (RTL8761B) USB Bluetooth 5.0 adapter via PyUSB.

Protocol reverse-engineered from com.qunyu.robolab APK.

Key fixes vs v1:
  - Advertising type ADV_IND (0x00, connectable) — robot ignores non-connectable
  - Simultaneous HCI scan during pairing to capture bid1/bid2 from robot response
...
# HCI opcodes  (OGF<<10 | OCF, little-endian in packet)
HCI_RESET                  = 0x0C03
...
MFR_ID_CONTROLLER = bytes([0x00, 0xC2])   # 0xC200 = 49664  — what we advertise
...
class BLEAdvertiserHCI:
    ...
    def __init__(self):
        self._dev         = None
        self._ep_in       = None   # interrupt IN  (events)
        self._ep_bulk_in  = None   # bulk IN  (ACL)
    ...
    def _hci_cmd(self, opcode: int, params: bytes, wait_ms: int = 1000) -> bool:
        """Send HCI command via USB control transfer, optionally drain one event."""
    ...
    def _drain_event(self, timeout: int = 500):
        """Read and discard one HCI event (command-complete / status)."""
    ...
    def _read_raw_event(self, timeout: int = 300) -> bytes | None:
        """Read one raw HCI event from interrupt endpoint, return bytes or None."""
        if self._ep_in is None:
            return None
        try:
    ...
    def _adv_params(self) -> bool:
        ...
        # ADV_IND = 0x00  (connectable undirected) — Android uses setConnectable(true)
        # interval 0x00A0 = 160 × 0.625 ms = 100 ms
        return self._hci_cmd(HCI_LE_SET_ADV_PARAMETERS, bytes([
            0xA0, 0x00,   # interval min
            0xA0, 0x00,   # interval max
            0x00,          # ADV_IND — connectable undirected (was 0x03, WRONG)
    ...
    def _adv_data(self, payload: bytes) -> bool:
        ...
        # AD structure: [length, 0xFF, mfr_id_lo, mfr_id_hi, ...payload]
    ...
    # ------------------------------------------------------------------
    # Scanning  (to capture robot's bid1/bid2 response during pairing)
    # ------------------------------------------------------------------
    ...
    def _parse_adv_report(raw: bytes, app_id1: int, app_id2: int) -> tuple | None:
        """
        Parse HCI_LE_Meta_Event (0x3E) / LE_Advertising_Report (subevent 0x02).

        USB HCI event format (no H4 indicator byte):
          [event_code, param_len, subevent, num_reports,
           event_type, addr_type, addr×6, data_len, data×N, rssi]
        """
        if len(raw) < 13:
            return None
        ...
        for _ in range(num_reports):
            if offset + 9 > len(raw):
                break
            # raw[offset]   = event_type
            # raw[offset+1] = address_type
            # raw[offset+2..+7] = address
    ...
    def _parse_ad_mfr(ad_data: bytes, app_id1: int, app_id2: int) -> tuple | None:
        ...
        while i < len(ad_data):
            ...
            if ad_type == 0xFF and ad_len >= 10:
                # manufacturer ID is little-endian 2 bytes after type
...
class RobotState:
    ...
    app_id1:   int  = 17
    app_id2:   int  = 34
    # Known robot IDs from PROGRESS.md (updated by pair() scan)
    bid1:      int  = 48
...
def build_command_14(c: int, d_val: int) -> bytes:
    """
    14-byte motor command using actual robot bid bytes and single-device mode.

    After pairing, the robot expects:
      data[0] = bid1, data[1] = bid2  (robot's device ID)
      data[6] = 0                     (single-device mode, NOT 0xFF broadcast)
    """
    rand = random.randint(1, 255)
    data = bytearray(14)
    data[0]  = robot.bid1 & 0xFF          # robot device ID
    data[1]  = robot.bid2 & 0xFF
    data[2]  = robot.app_id1
    data[3]  = robot.app_id2
    data[4]  = c     & 0xFF
    data[5]  = d_val & 0xFF
    data[6]  = 0                           # single-device mode (0xFF = broadcast, ignored after pair)
...
Uses raw HCI commands on the ASUS USB-BT500 USB adapter (no Android needed).
...
async def pair() -> str:
    ...
    # --- Phase 1: advertise verify code for ~2 s to wake robot ---
    for _ in range(14):
    ...
    def _do_scan():
        result = advertiser.scan_for_robot(robot.app_id1, robot.app_id2, duration=5.0)
        scan_result[0] = result

    scan_thread = threading.Thread(target=_do_scan, daemon=True)
    scan_thread.start()

    # Keep advertising verify code while scanning
    t_end = time.time() + 5.5
