...
The robot uses QUNYU BLE protocol where the controller broadcasts
commands as BLE advertising packets with manufacturer-specific data.
The robot passively scans for these packets.

This version bypasses macOS CoreBluetooth entirely and sends raw HCI
commands directly to an ASUS USB-BT500 (RTL8761B) USB Bluetooth 5.0
adapter using PyUSB, allowing custom manufacturer data in advertising.
...
try:
    ...
    _USB_BACKEND = None  # pyusb will auto-detect (works if libusb is installed system-wide)
...
# HCI opcodes (little-endian, OCF|OGF<<10)
HCI_RESET                       = 0x0C03  # OGF=0x03, OCF=0x0003
HCI_LE_SET_ADV_PARAMETERS       = 0x2006  # OGF=0x08, OCF=0x0006
HCI_LE_SET_ADV_DATA             = 0x2008  # OGF=0x08, OCF=0x0008
HCI_LE_SET_ADV_ENABLE           = 0x200A  # OGF=0x08, OCF=0x000A

# ASUS USB-BT500 identifiers
# PID varies by firmware/region: 0x190E is listed in some datasheets but
# the RTL8761B-based BT500 on macOS enumerates as 0x1BF6 (confirmed via
# system_profiler). Both PIDs are tried; class-scan fallback catches others.
ASUS_VENDOR_ID   = 0x0B05
ASUS_PRODUCT_ID  = 0x1BF6   # actual PID seen on macOS (was incorrectly 0x190E)
ASUS_PRODUCT_ID_ALT = 0x190E  # alternate PID listed in some docs

# USB Bluetooth device class identifiers
BT_CLASS    = 0xE0
BT_SUBCLASS = 0x01
BT_PROTOCOL = 0x01

# HCI USB endpoint
HCI_CMD_ENDPOINT   = 0x00   # Control endpoint (EP0) — used for HCI commands
HCI_EVENT_ENDPOINT = 0x81   # Interrupt IN endpoint


def _build_hci_cmd(opcode: int, params: bytes) -> bytes:
    """Build an HCI command packet.

    Format for USB bulk OUT (or control transfer):
      opcode_lo, opcode_hi, total_param_length, ...params
    """
    return bytes([opcode & 0xFF, (opcode >> 8) & 0xFF, len(params)]) + params


class BLEAdvertiserHCI:
    """Manages BLE advertising by sending raw HCI commands over USB."""
    ...
    def _find_device(self):
        """Find the USB Bluetooth adapter. Returns usb.core.Device or None."""
        kw = {"backend": _USB_BACKEND} if _USB_BACKEND is not None else {}

        # 1. Try primary PID (0x1BF6 — confirmed on macOS with RTL8761B)
        dev = usb.core.find(idVendor=ASUS_VENDOR_ID, idProduct=ASUS_PRODUCT_ID, **kw)
        if dev is not None:
            logger.info(f"Found ASUS USB-BT500 at {ASUS_VENDOR_ID:#06x}:{ASUS_PRODUCT_ID:#06x}")
            return dev

        # 2. Try alternate PID (0x190E — some firmware/region variants)
        dev = usb.core.find(idVendor=ASUS_VENDOR_ID, idProduct=ASUS_PRODUCT_ID_ALT, **kw)
        if dev is not None:
            logger.info(f"Found ASUS USB-BT500 at {ASUS_VENDOR_ID:#06x}:{ASUS_PRODUCT_ID_ALT:#06x}")
            return dev

        # 3. Fall back: scan all USB devices for Bluetooth class
                ...
                for cfg in dev:
                    for intf in cfg:
                        if (intf.bInterfaceClass    == BT_CLASS and
                            ...
                            logger.info(
                                f"Found BT adapter via class scan: "
    ...
    def _ensure_init(self) -> bool:
        ...
        with self._lock:
            ...
            if dev is None:
                logger.error("No USB Bluetooth adapter found")
                return False

            # Detach kernel driver if needed (mainly Linux; macOS may silently fail)
            try:
                if dev.is_kernel_driver_active(0):
                    dev.detach_kernel_driver(0)
                    logger.info("Detached kernel driver from interface 0")
            except Exception as e:
                logger.debug(f"detach_kernel_driver: {e} (may be normal on macOS)")
            ...
            # Locate the interrupt IN endpoint (0x81)
            ...
            if ep_in is None:
                logger.warning("Interrupt IN endpoint not found; event reads will be skipped")

            self._dev = dev
            self._intf = intf
            self._ep_in = ep_in

            # Send HCI Reset and wait for Command Complete event
            ok = self._send_hci_reset()
            if not ok:
                logger.error("HCI Reset failed")
                return False

            self._initialized = True
            logger.info("BLEAdvertiserHCI initialized")
            return True
    ...
    def _send_hci_cmd(self, opcode: int, params: bytes, timeout: int = 2000) -> bool:
        """Send an HCI command via USB control transfer and read event response."""
        if self._dev is None:
            return False

        pkt = _build_hci_cmd(opcode, params)
        try:
            # HCI commands go over the USB control endpoint (bmRequestType=0x20,
            # bRequest=0x00) per the Bluetooth USB transport spec (HCI_CMD indicator=0x00).
            ...
            logger.debug(f"HCI cmd sent: opcode={opcode:#06x} params={params.hex()}")
        except Exception as e:
            logger.error(f"HCI ctrl_transfer error: {e}")
            return False

        # Read Command Complete event from interrupt IN endpoint
        self._read_hci_event(timeout=timeout)
        return True

    def _read_hci_event(self, timeout: int = 1000):
        """Read and discard one HCI event from the interrupt IN endpoint."""
        if self._ep_in is None:
            return
        try:
            data = self._ep_in.read(64, timeout=timeout)
            logger.debug(f"HCI event: {bytes(data).hex()}")
        except usb.core.USBTimeoutError:
            logger.debug("HCI event read timed out (normal if adapter is busy)")
        except Exception as e:
            logger.debug(f"HCI event read error: {e}")

    def _send_hci_reset(self) -> bool:
        """Send HCI_Reset and wait for Command Complete."""
        logger.info("Sending HCI Reset...")
        return self._send_hci_cmd(HCI_RESET, b"")

    # ------------------------------------------------------------------
    # Advertising
    # ------------------------------------------------------------------

    def _set_adv_parameters(self) -> bool:
        """LE_Set_Advertising_Parameters — non-connectable undirected, ~100ms interval."""
        # interval in units of 0.625ms; 0x00A0 = 160 * 0.625ms = 100ms
        interval_lo = 0xA0
        interval_hi = 0x00
        params = bytes([
            interval_lo, interval_hi,   # advertising_interval_min
            interval_lo, interval_hi,   # advertising_interval_max
            0x03,                        # advertising_type: non-connectable undirected (ADV_NONCONN_IND)
            0x00,                        # own_address_type: public
            0x00,                        # peer_address_type: public
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  # peer_address (ignored)
            0x07,                        # advertising_channel_map: all 3 channels
            0x00,                        # advertising_filter_policy: none
        ])
        return self._send_hci_cmd(HCI_LE_SET_ADV_PARAMETERS, params)

    def _set_adv_data(self, payload: bytes) -> bool:
        """LE_Set_Advertising_Data — embed payload as manufacturer specific AD type (0xFF)."""
        # AD structure: [length, type, manufacturer_id_lo, manufacturer_id_hi, ...payload]
        mfr_id = MANUFACTURER_ID  # [0x00, 0xC2]
        ad_content = bytes([0xFF]) + mfr_id + payload  # type + id + payload
        ad_len = len(ad_content)  # length field = number of bytes following it

        # Full AD structure: [ad_len, ...ad_content]
        ad_struct = bytes([ad_len]) + ad_content

        # Total advertising data length (must be <= 31 bytes)
        total_ad_len = len(ad_struct)
        if total_ad_len > 31:
            logger.warning(f"AD data too long ({total_ad_len} bytes), truncating to 31")
            ad_struct = ad_struct[:31]
            total_ad_len = 31

        # Pad to 31 bytes
        pad = bytes(31 - total_ad_len)

        # HCI params: [significant_len (1 byte), 31 bytes of ad data]
        hci_params = bytes([total_ad_len]) + ad_struct + pad
        return self._send_hci_cmd(HCI_LE_SET_ADV_DATA, hci_params)

    def _set_adv_enable(self, enable: bool) -> bool:
        """LE_Set_Advertise_Enable."""
        return self._send_hci_cmd(HCI_LE_SET_ADV_ENABLE, bytes([0x01 if enable else 0x00]))

    # ------------------------------------------------------------------
    # Public interface (same as BLEAdvertiser)
    # ------------------------------------------------------------------
    ...
    def advertise(self, payload: bytes) -> bool:
        ...
        # Disable advertising before changing parameters/data
        self._set_adv_enable(False)

        if not self._set_adv_parameters():
            logger.error("Failed to set advertising parameters")
            return False

        if not self._set_adv_data(payload):
            logger.error("Failed to set advertising data")
            return False

        if not self._set_adv_enable(True):
    ...
    def stop(self):
        """Stop advertising and release USB resources."""
...
Uses raw HCI commands sent directly to an ASUS USB-BT500 (RTL8761B) USB Bluetooth 5.0
adapter, bypassing macOS CoreBluetooth to allow custom manufacturer data in advertising.
...
async def pair() -> str:
    ...
    Broadcasts the verification code via BLE advertising using raw HCI over USB.
    The robot will respond by advertising its device ID.
    Make sure the robot is powered on with Bluetooth enabled.
    The ASUS USB-BT500 adapter must be plugged in.
    """
    if robot.connected:
        return "Already connected. Use disconnect() first."

    # Initialize USB BLE adapter
    if not advertiser._ensure_init():
        return (
            "ERROR: USB Bluetooth adapter not found or failed to initialize.\n"
            "Make sure the ASUS USB-BT500 adapter is plugged in.\n"
    ...
    return (
        "Pairing broadcast sent via USB HCI! Robot should now be responding.\n"
...
async def status() -> str:
    ...
    return (
        ...
        f"HCI Adapter: {'initialized' if advertiser._initialized else 'not initialized'}"
    )
